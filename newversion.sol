...
contract DividendDistributor is IDividendDistributor {
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external override onlyToken {
        ...
        emit debug("setDistributionCriteria");
    ...
    event logDepositBalanceBefore(uint256 balance);
    event logDepositBuySize(uint256 balance);

    function deposit()external payable override onlyToken {
        
        emit debug("deposit called");
        
        uint256 balanceBefore = BUSD.balanceOf(address(this));
        emit logDepositBalanceBefore (balanceBefore);
        
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(BUSD);

        emit debug("deposit will buy" );
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(shouldDistribute(shareholders[currentIndex])){
                emit debug("process true, will call distributeDividend" );
                distributeDividend(shareholders[currentIndex]);
            }
            else
            {
                emit debug("process not");
            }
...
contract RudolphCoin is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch { emit debug("got on catch if idivint "); } }
...
//        emit Transfer(sender, recipient, amount);
    ...
    event swapBackDebug( uint256 dynamicLiquidityFee, uint256 amountToLiquify,  uint256 amountToSwap  );
    ...
    event logButTokens (uint256 amount, address to);

    function buyTokens(uint256 amount, address to) internal swapping {
        
        ///dev/null/DEBUG
        emit logButTokens(amount, to);
        ///dev/null/DEBUG
